...
系统已经完成了旧 pipeline 到 LLM tool-calling runtime 的迁移，接下来的目标是把系统打磨成一个边界清晰、可评估、可回放、可演示、可扩展的 Agent 工程作品。

长期优化原则：

1. **Runtime 单一，Harness 多样**：生产 runtime 只保留 LLM tool-calling 主路径；离线演示、scripted eval、trace replay 都是显式 harness。
2. **安全边界优先于通过率**：任何写操作都必须经过 `ToolGateway`、`WriteActionGuard` 和用户确认。
3. **先观测，再优化**：prompt、tool schema、guard、runtime 的优化由 trace、eval report、failure category 支撑。
4. **每个 Phase 都可停止、可验证、可交接**：避免一次性大重构。
...
每个 Phase 结束时写一段 review，不只看测试是否通过，还要回答架构是否更清晰、风险是否降低。

建议 review 覆盖：本 phase 的目标、改了哪些模块、架构边界是否保持单一 runtime 路径、写工具是否仍全部经过 gateway+guard+confirmation、测试和 eval 结果、遗留的债和下一 phase 优先级。
...
建立真实模型能力基线。Phase 9 的重点不是提高通过率，而是让每次 live eval 失败都能被准确归因。

### 现状 Review

Scripted eval 已经稳定，live eval 也能通过 `--live` 触发，但 live 和 scripted 结果之间还缺一层分析契约。现有的 failure label 偏 eval 断言层，缺少 prompt/schema/model/provider/data/policy 的工程归因。
...
Phase 7、Phase 8 和 Phase 9 已完成，是后续所有增长的地基。Phase 11-13 可以穿插推进，但建议在 Phase 10 prompt/schema baseline 稳定后再大规模扩 tau coverage。
