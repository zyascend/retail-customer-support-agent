...
1. Phase 9：Live Eval Baseline and Failure Taxonomy。
2. Phase 10：Prompt and Tool Schema Optimization。
3. Phase 11-13：在 Phase 9/10 的 baseline 稳定后扩展 tau coverage。

Phase 7 和 Phase 8 已完成，是后续所有增长的地基。Phase 11-13 可以穿插推进，但不建议在 Phase 9 baseline 前大规模扩 tau coverage。

## 下一份实施计划建议

下一份可执行 plan 建议聚焦 Phase 9，文件命名：

`docs/superpowers/plans/2026-06-14-phase9-live-eval-baseline-failure-taxonomy.md`

Phase 9 plan 应拆成这些任务：

1. 固定 live eval baseline 的 case set、model/backend 和指标口径。
