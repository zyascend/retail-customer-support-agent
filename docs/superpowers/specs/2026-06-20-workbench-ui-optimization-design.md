...
**左侧面板**：点击左侧边界的 `◀` 按钮收起/展开
- 收起后: Left 栏宽度为 0 (`w-0`)，中间区域占满右侧空间
- 展开时: Left 栏宽度 `w-72`
- 收起按钮固定在左栏右侧边缘或 Header 区域

**右侧面板**：点击右侧边界的 `▶` 按钮收起/展开
- 收起后: Right 栏宽度为 0 (`w-0`)，中间区域占满左侧空间  
- 展开时: Right 栏宽度 `w-80`
- 收起按钮固定在右栏左侧边缘或 Header 区域

**面板展开/收起规则**：

| 面板 | 默认状态 | 收起后 | 展开后 |
|------|---------|--------|--------|
| 左侧 | 展开 | w-0, overflow-hidden, transition | w-72 |
| 右侧 | 展开 | w-0, overflow-hidden, transition | w-80 |
| 中间 | flex-1 | flex-1 (空间变大) | flex-1 |

面板状态使用 `useState` 管理，保存在组件本地；所有面板可独立收起互不影响。
...
| **Left** | RunControl (下拉 + 按钮 + 消息) | Case 分组折叠树 (纯列表) + 可收起 |
| **Left 边界** | — | 新增 `◀` 收起按钮 |
| **Middle 顶部** | — | 新增 Step/Run/Reset 行内按钮组 |
| **Middle 底部-消息** | Left 栏内 | 移到 Conversation 下方 |
| **Right** | Inspector (JSON) | 事件详情卡片 (结构化 UI) + 可收起 |
| **Right 边界** | — | 新增 `▶` 收起按钮 |
...
| 分组初始状态 | 全部展开 |
| **收起面板** | 点击左栏右侧 `◀` 按钮，面板平滑缩回 |
| **展开面板** | 点击左栏边缘或 Header 的 `▶` 按钮，面板平滑伸出 |
...
收起时右侧面板回到 `w-0`，中间区域弹性填满剩余空间。展开时回到 `w-80`。面板状态保存在 `App.tsx` 的 `useState` 中，不与后端交互。
...
| `CollapseButton` | 新增 | 通用面板收起/展开按钮，接受 `collapsed` + `onToggle` props |
| `EventDetailPanel` | 新增 | 右侧事件详情容器，包含收起按钮，路由到不同卡片 |
...
2. 修改 `App.tsx` 布局：左栏替换为 CaseTree，控制按钮迁移，添加面板收起/展开状态管理
3. 创建 `CollapseButton` 组件并接入左右面板
4. 创建卡片组件：`ToolCallCard` → `StepCard` → `MessageCard` → `WriteAuditCard`
5. 创建 `EventDetailPanel` 替换 `Inspector`（含收起按钮）
6. 对接左栏选中事件与右侧详情联动
7. 移除旧代码：RunControl 中的 case 列表、Inspector JSON 展示、guard_blocks 独立展示
8. 测试验证