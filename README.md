...
用于把 live eval 的失败样本沉淀为 bad case、扩展可泛化变体、提升为 golden，并持续做回归检查。

**适用场景**
- 你刚跑完 `--live` eval，想把真实失败沉淀下来
- 你要把某个已知 bad case 升级成长期回归用例
- 你改了 prompt / guard / runtime，想确认 golden 没被改坏

**推荐流程**
1. 先跑 live eval，拿到 report JSON
2. 用 `flywheel collect` 收集失败 case 到 `cases/bad_cases/<date>.yaml`
3. 如 case 带 `seed + variant_type`，可用 `flywheel generate` 生成 gate 变体
4. 用 `flywheel golden promote --confirm` 把关键 case 放进 `cases/golden.yaml`
5. 用 `flywheel check` 跑 golden 回归；若出现 `regression` / `missing`，退出码为 `1`
...
# 4. 把关键 case 提升进 golden（当前必须显式加 --confirm）
...
- `promote` 当前是非交互式确认，漏掉 `--confirm` 会直接失败
