...
| Offline demo harness | 可用但需边界收敛 | `offline_demo=True` 下存在规则解析逻辑 |

当前主要漂移点：

| 漂移点 | 影响 | 处理方向 |
|--------|------|----------|
| `offline_demo` 规则解析仍在 `AgentRuntime` 文件内 | 读代码时容易误以为 runtime 仍有 deterministic fallback | Phase 7 拆成显式 harness module |
| scripted eval 默认走 offline demo harness | scripted pass rate 不能代表真实 LLM 能力 | Phase 7/9 明确 eval backend 语义 |
| `WriteActionGuardResult.block_context` 未实现 | guard block 给 LLM 的解释上下文不够结构化 | Phase 8 补齐 |
| trace/workbench 仍输出 `current_intent`, `slots`, `policy_decision` 占位 | 展示层让旧架构概念继续存在 | Phase 7 清理或隔离为 compat 字段 |
| `AgentLoop` 有 auto-load 和 premature-refusal correction | 提升安全和通过率，但比“全由 LLM 选择工具”更保守 | Phase 8/10 把它们文档化并用 eval 监控 |
...
1. Phase 7：Runtime / Harness Boundary Hardening。
2. Phase 8：Structured Guard Context。
3. Phase 9：Live Eval Baseline and Failure Taxonomy。
4. Phase 10：Prompt and Tool Schema Optimization。

这四个 phase 是后续所有增长的地基。Phase 11-13 可以穿插推进，但不建议在 Phase 7-9 前大规模扩 tau coverage。

## 下一份实施计划建议

下一份可执行 plan 建议聚焦 Phase 7，文件命名：

`docs/superpowers/plans/2026-06-14-phase7-runtime-harness-boundary-hardening.md`

Phase 7 plan 应拆成这些任务：

1. 新增 `OfflineDemoHarness` 并迁移 demo-only parser。
2. 精简 `AgentRuntime`，只保留 orchestration。
3. 细化 eval backend 命名。
4. 清理 Workbench/trace 的 legacy state 字段展示。
5. 更新 README 和 portfolio architecture。
6. 跑 full pytest、ruff、curated scripted eval。

Phase 7 完成后，再进入 Phase 8 的 `block_context` 实施计划。
